...
def _fixture(count: int = 4) -> tuple[C1UModel, dict[str, object], C1URuntime]:
    bank = build_s1_bank()
    cache = build_independent_card_cache(bank, _encoder, source_identity={"model": "fixture"})
    runtime = C1URuntime(bank, cache, address_width=16)
...
def test_forward_contract_shapes_and_integrity() -> None:
    ...
    assert model.integrity_report()["learned_write_gate_parameters"] == []
    assert all("gate" not in name.casefold() for name, _ in model.transition.named_parameters())
    assert model.transition.operator[-1].out_features == model.config.payload_width
...
def test_inactive_transition_is_exact_identity() -> None:
    model, batch, runtime = _fixture()
    with torch.no_grad():
        boundary = model.boundary(**runtime.forward_inputs(batch))
        trajectory = model.forward_from_boundary(boundary, return_trajectory=True)["trajectory"]
    inactive = ~boundary.operation_active
    assert bool(inactive.any())
    for row, step in inactive.nonzero(as_tuple=False).tolist():
        assert torch.equal(trajectory[row, step], trajectory[row, step + 1])
