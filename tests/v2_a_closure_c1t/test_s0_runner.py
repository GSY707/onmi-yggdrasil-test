...
from yggdrasil_v2.v2_a.closure_c1t import artifacts, contract
from yggdrasil_v2.v2_a.closure_c1t.model import C1TConfig
...
def _prepare_repo(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr(contract, "S1_PREFLIGHT_ROOT", Path("tmp/s1-preflight"))
    monkeypatch.setattr(contract, "S1_PREFLIGHT_LEASE", Path("tmp/s1-preflight.lease.jsonl"))
    monkeypatch.setattr(contract, "S1_ROOT", Path("artifacts/s1"))
    monkeypatch.setattr(contract, "S1_LEASE", Path("artifacts/s1.lease.jsonl"))
    return tmp_path


def _config() -> C1TConfig:
    return C1TConfig(
        source_width=12,
        payload_width=16,
        address_width=16,
...
def test_zero_training_runner_has_no_optimizer_or_checkpoint_write_path() -> None:
    source = (
        ...
        / "src/yggdrasil_v2/v2_a/closure_c1t/s0.py"
...
def test_postclaim_fault_is_crash_sealed_and_not_retriable(
    ...
    assert result["status"] == "CRASH_V2_A_C1T_S0_PREFLIGHT"
