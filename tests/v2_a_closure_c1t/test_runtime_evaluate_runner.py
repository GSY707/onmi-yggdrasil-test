...
from yggdrasil_v2.v2_a.closure_c1t.artifacts import SOURCE_FILES, source_hashes
from yggdrasil_v2.v2_a.closure_c1t.evaluate import (
...
def _runtime() -> tuple[list[dict[str, object]], IndependentCardCache, C1TRuntime]:
    bank = build_s1_bank()
    cache = build_independent_card_cache(
        bank, _encoder, source_identity={"model": "fixture", "revision": "fixed"}
    )
    return bank, cache, C1TRuntime(bank, cache, address_width=16)
...
def test_random_model_fails_s1_but_structural_invariance_passes() -> None:
    ...
    assert report["status"] == "FAIL_C1T_S1_QUALIFICATION"
        ...
        rows.append(
            {
                ...
                "support_prediction_indices": [record["answer_index"], record["answer_index"]],
                "support_expected_answer_indices": [record["answer_index"], record["answer_index"]],
                "support_flip_correct": [True, True],
            }
        )
    report = qualify_s1(
        {"rows": rows, "invariance": {"passed": True}}, require_frozen_cardinality=True
    )
    assert report["passed"] is True
    assert report["status"] == "PASS_C1T_S1"
