...
MODES = ("baseline_parallel", "supervised_no_teacher", "supervised_teacher", "token_aligned_teacher")
...
def stage_y_config(config: StageAAConfig) -> stage_y.StageYConfig:
...
def stage_v_config(config: StageAAConfig) -> stage_v.StageVConfig:
...
def stage_w_config(config: StageAAConfig) -> stage_w.StageWConfig:
...
def count_targets(meta: list[dict[str, object]], config: StageAAConfig) -> torch.Tensor:
...
@dataclass
class StageAASemanticSet:
    semantic: stage_v.SemanticPixelSet
    count: torch.Tensor

    def subset(self, indices: list[int]) -> "StageAASemanticSet":
        return StageAASemanticSet(
            semantic=self.semantic.subset(indices),
            count=self.count[indices],
        )

    def to(self, device: torch.device) -> "StageAASemanticSet":
        return StageAASemanticSet(
...
def build_data(config: StageAAConfig) -> tuple[dict[str, StageAASemanticSet], dict[str, object]]:
    semantic, stats = stage_v.build_semantic_data(stage_v_config(config))
    data = {
        split_name: StageAASemanticSet(
...
def random_batch(data: StageAASemanticSet, *, rng: random.Random, batch_size: int, device: torch.device) -> StageAASemanticSet:
...
class CountTableDecoder(nn.Module):
    def __init__(self, config: StageAAConfig) -> None:
        super().__init__()
...
class DistillProjector(nn.Module):
    def __init__(self, config: StageAAConfig, source_tokens: int) -> None:
        super().__init__()
...
class SupervisionHeads(nn.Module):
    def __init__(self, config: StageAAConfig, *, use_teacher: bool, use_token_alignment: bool) -> None:
        super().__init__()
        self.config = config
    ...
    def explicit_loss(self, reps: dict[str, torch.Tensor], batch: StageAASemanticSet) -> tuple[torch.Tensor, dict[str, float]]:
...
def train_model(
    ...
    train: StageAASemanticSet,
    val: StageAASemanticSet,
    config: StageAAConfig,
    *,
    candidate_pool: stage_q.CandidatePool,
    seed: int,
    ...
    use_supervision = mode in {"supervised_no_teacher", "supervised_teacher", "token_aligned_teacher"}
    torch.manual_seed(seed + 17)
    heads = (
        SupervisionHeads(
            config,
            use_teacher=mode in {"supervised_teacher", "token_aligned_teacher"},
            use_token_alignment=mode == "token_aligned_teacher",
        ).to(device)
        if use_supervision
        else None
    )
...
class AnswerProbe(nn.Module):
    def __init__(self, config: StageAAConfig, class_count: int) -> None:
        super().__init__()
...
def answer_labels(data: StageAASemanticSet, label_by_answer: dict[str, int], device: torch.device) -> torch.Tensor:
...
def run_experiment(config: StageAAConfig, output_path: Path) -> dict[str, object]:
    started = time.perf_counter()
    random.seed(config.seed)
    torch.manual_seed(config.seed)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"stage_aa_token_alignment_diagnostics device={device} seed={config.seed}", flush=True)
    if device.type == "cuda":
    ...
    output = {
        "experiment": "omni_transformer_stage_aa_token_alignment_diagnostics",
...
def aggregate_runs(runs: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        "experiment": "omni_transformer_stage_aa_token_alignment_diagnostics_sweep",
        "run_count": len(runs),
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("artifacts/omni_transformer_stage_aa_token_alignment_diagnostics/result.json"))
    parser.add_argument("--output-dir", type=Path, default=Path("artifacts/omni_transformer_stage_aa_token_alignment_diagnostics/sweep_runs"))
    parser.add_argument("--aggregate", type=Path, default=Path("artifacts/omni_transformer_stage_aa_token_alignment_diagnostics/sweep_results.json"))
    parser.add_argument("--sweep", action="store_true")
    parser.add_argument("--seeds", default="20260701,20260702")
    parser.add_argument("--modes", default="baseline_parallel,supervised_no_teacher,supervised_teacher,token_aligned_teacher")
    ...
    if args.sweep:
        ...
        for seed in parse_csv_ints(args.seeds):
            config = StageAAConfig(**{**asdict(base_config), "seed": seed})
