...
"""Non-consuming, stage-aware inspection for the frozen C1U chain."""
...
from .runtime import C1URuntime, build_independent_card_cache
from .s1 import audit_s0_predecessor
...
def _structural_smoke() -> dict[str, Any]:
    ...
    model = C1UModel(
        C1UConfig(source_width=cache.source_width, payload_width=32, ffn_width=64)
    )
    model.train()
    ...
    checks = {
        ...
        "gate_free_transition": model.integrity_report()["learned_write_gate"] is False
        and model.integrity_report()["gate_free_target_overwrite"] is True,
...
def inspect_successor(repo_root: Path) -> dict[str, Any]:
    ...
    checks = {
        ...
        "sealed_s0_predecessor": predecessor["passed"],
