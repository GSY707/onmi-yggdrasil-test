...
"""Deterministic complete-factorial training for C1U S1.
...
from .runtime import C1URuntime
...
class RuntimeBatchProvider:
    ...
    def __init__(self, runtime: C1URuntime) -> None:
    ...
    def get_batch(
        ...
        if len(key) != contract.S1_BATCH_SIZE:
            raise ValueError("C1U S1 batches must contain exactly eight records")
...
def build_optimizer(model: C1UModel, spec: TrainSpec) -> torch.optim.Optimizer:
...
def load_endpoint(
    ...
    model = C1UModel(C1UConfig(**dict(payload["config"])))
