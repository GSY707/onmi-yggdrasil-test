...
"""Independent-card cache and batch runtime for C1U.
...
class C1URuntime:
    ...
    def get_batch(self, example_ids: Sequence[str]) -> dict[str, Any]:
        ...
        for example_id in example_ids:
            if example_id not in self.records:
                raise KeyError(f"unknown C1U example_id: {example_id}")
    ...
    def forward_inputs(batch: Mapping[str, Any]) -> dict[str, Tensor]:
        ...
        if missing:
            raise KeyError(f"batch is missing C1U forward fields: {missing}")
