...
"""C1U source closure, single-use leases, snapshots, and evidence seals."""
...
# S1 seals the original S0 scientific core plus the newly authorized execution
# layer.  README/status/result-review documents written after a terminal S1
# remain outside this immutable execution closure.
SOURCE_FILES = (
    contract.DESIGN_DOC,
    contract.S0_EXECUTION_DOC,
    contract.S0_RESULT_REVIEW_DOC,
    contract.S1_EXECUTION_DOC,
    Path("experiments/v2_a_closure_c1u.py"),
    ...
    Path("src/yggdrasil_v2/v2_a/closure_c1u/runner.py"),
    Path("src/yggdrasil_v2/v2_a/closure_c1u/runtime.py"),
    Path("src/yggdrasil_v2/v2_a/closure_c1u/s0.py"),
...
def source_hashes(repo_root: Path) -> dict[str, str]:
    ...
    if missing:
        raise FileNotFoundError(f"C1U source identity is incomplete: {missing}")
