...
"""External C1T supervision and causal interventions.

Answers, valid choices, and support-slot ledgers are consumed only here.  They
are never passed to :class:`C1TModel.forward`.
"""
...
from .model import C1TModel, PartitionedBoundaryOutput
...
def _forward_fields(batch: Mapping[str, Any]) -> dict[str, Tensor]:
    ...
    if missing:
        raise KeyError(f"C1T batch is missing forward fields: {missing}")
...
def compute_training_loss(
    ...
    *,
    ...
    if "counterfactual_indices" not in batch:
        raise KeyError("C1T causal objective requires complete-group counterfactual_indices")
