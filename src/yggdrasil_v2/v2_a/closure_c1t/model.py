...
"""C1T causally-partitioned workspace.
...
class IndependentCardEncoder(nn.Module):
    ...
    def __init__(self, config: C1TConfig) -> None:
        super().__init__()
        self.config = config
...
class PartitionedBoundary(nn.Module):
    ...
    def forward(
        ...
        if slots != self.config.slots:
            raise ValueError(f"C1T requires exactly {self.config.slots} padded object slots")
...
class TargetOnlyTransition(nn.Module):
    """Shared recurrent operation which writes only the registered target."""
    ...
    def __init__(self, config: C1TConfig) -> None:
        ...
        self.operator = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(config.ffn_width, width + 1),
        )
    ...
    def forward(
        ...
        proposal = self.operator(torch.cat((source, target, operation_state), dim=-1))
        proposed_target, gate_logit = proposal[:, :-1], proposal[:, -1:]
        target_delta = torch.sigmoid(gate_logit) * (proposed_target - target)
...
class C1TModel(nn.Module):
    """Causal-card workspace whose deployment graph is source-card-only."""

    def __init__(self, config: C1TConfig = C1TConfig()) -> None:
        super().__init__()
        ...
        if config.slots < 3 or config.operations < 2:
            raise ValueError("C1T requires at least three slots and two operations")
        if config.answer_classes != contract.ANSWER_CLASSES:
            raise ValueError("C1T freezes raw A-I answer logits")
    ...
    def integrity_report(self) -> dict[str, Any]:
        ...
        return {
            ...
            "architecture": "causally-partitioned-workspace",
