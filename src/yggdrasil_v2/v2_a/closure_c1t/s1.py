...
"""Fail-closed single-use preflight and formal execution for C1T S1."""
...
from .runtime import C1TRuntime, audit_independent_card_cache
...
def _runtime_bundle(
    repo_root: Path,
) -> tuple[list[dict[str, Any]], C1TRuntime, dict[str, Any]]:
    records = _s0_records(repo_root)
...
def _model_config(source_width: int, override: C1TConfig | None) -> C1TConfig:
    config = C1TConfig(source_width=source_width) if override is None else override
...
def run_s1_preflight(
    ...
    *,
    ...
    preclaim_checks = {
        ...
        "user_authorization": contract.S1_USER_AUTHORIZATION == "开始S1阶段",
    }
    ...
    try:
        if fault_at == "after_claim":
            raise RuntimeError("injected C1T S1 preflight fault after claim")
        ...
        with StepJournal(root / "disposable-training-progress.jsonl") as journal:
            benchmark = train_fixed_endpoint(
                ...
                provider,
                benchmark_schedule,
                identity="DISPOSABLE_C1T_S1_PREFLIGHT_BENCHMARK",
        ...
        gates = {
            ...
            and contract.S1_USER_AUTHORIZATION == "开始S1阶段",
        ...
        if fault_at == "before_result":
            raise RuntimeError("injected C1T S1 preflight fault before result")
        result = {
            "schema_version": f"{contract.SCHEMA_PREFIX}.s1-preflight-result.v1",
            "identity": contract.S1_PREFLIGHT_IDENTITY,
            "stage": "s1-preflight",
            "status": "PASS_V2_A_C1T_S1_PREFLIGHT"
            if passed
            else "FAIL_V2_A_C1T_S1_PREFLIGHT",
            "passed": passed,
...
def audit_s1_preflight(repo_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        checks = {
            ...
            "status": result.get("status") == "PASS_V2_A_C1T_S1_PREFLIGHT",
            "passed": result.get("passed") is True,
            "authorization": result.get("authorizes")
...
def run_s1(
    ...
    *,
    ...
    try:
        if fault_at == "after_claim":
            raise RuntimeError("injected C1T S1 fault after claim")
        snapshot_sources(repo_root, root)
        write_json(root / "contract-manifest.json", contract.manifest())
        write_json(
            root / "preclaim-audit.json",
        ...
        if fault_at == "after_training":
            raise RuntimeError("injected C1T S1 fault after training")
        ...
        if fault_at == "before_result":
            raise RuntimeError("injected C1T S1 fault before result")
        result = {
            "schema_version": f"{contract.SCHEMA_PREFIX}.s1-result.v1",
            "identity": contract.S1_IDENTITY,
            "stage": "s1",
            "status": "PASS_V2_A_C1T_S1_OVERFIT32"
            if passed
            else "FAIL_V2_A_C1T_S1_QUALIFICATION",
            "passed": passed,
...
def audit_s1_root(repo_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        checks = {
            ...
            in {
                "PASS_V2_A_C1T_S1_OVERFIT32",
                "FAIL_V2_A_C1T_S1_QUALIFICATION",
                "CRASH_V2_A_C1T_S1",
            },
            "source_hashes": result.get("source_hashes") == current_hashes,
            "source_identity": result.get("source_identity")
            ...
            "endpoint": (
                result.get("status") == "CRASH_V2_A_C1T_S1"
                and not endpoint
            )
            or (
