...
from .a1_18_train import load_a118_deployment


INTERVENTION_SCHEMA = "yggdrasil.v2-a1.18.auxiliary-stripped-interventions.v1"


@torch.no_grad()
def run_a118_interventions(
    checkpoint: Path,
    data_dir: Path,
    ...
    *,
    ...
    if not formal.get("passed"):
        raise RuntimeError("A1.18 interventions are forbidden before formal passes")
    ...
    for name, pairs in pairs_by_variant.items():
        ...
        results[name] = _evaluate_variant(
            ...
            old_records=(
                old
                if name in {"replacement", "deletion", "operation_shuffle"}
                else None
            ),
        )
