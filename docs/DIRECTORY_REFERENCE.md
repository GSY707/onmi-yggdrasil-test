...
| `experiments/omni_transformer_stage_avj_latent_reasoning_core.py` | Stage AV-J 潜空间状态推理核心训练入口：source scene record + text operation 到 latent workspace，再输出 target record、answer tokens、active read/process trace 指标，含 checkpoint/resume、AMP 和 GPU 常驻数据 |
| `experiments/omni_transformer_stage_avjb_trace_verifier.py` | Stage AV-J-B 长链 trace/verifier 训练入口：在 AV-J 基础上加入 candidate mask、count value、copy-vs-update gate、record verifier、字段级 target 指标，并后移/降权 answer loss |
| `experiments/omni_transformer_stage_avjc_rl_verifier_reward.py` | Stage AV-J-C RL/verifier reward 训练入口：复用 AV-J-B 结构，采样 target record/candidate/count，用 oracle verifier reward 和 self-critical baseline 直接优化 record-level 状态改写 |
...
| `tests/test_stage_avj_latent_reasoning_core.py` | Stage AV-J 数据与模型 schema 测试：验证 record edit 确实发生、三任务族覆盖、模型 forward/loss/metrics 字段可用 |
| `tests/test_stage_avjb_trace_verifier.py` | Stage AV-J-B rich trace 测试：验证 candidate mask、count value、copy gate、rich loss、模型 forward 和 metrics schema |
| `tests/test_stage_avjc_rl_verifier_reward.py` | Stage AV-J-C RL/verifier reward 测试：验证 RL schedule、policy-gradient reward loss、RL stats 和 target head 反传 |
...
| `artifacts/omni_transformer_stage_avj_latent_reasoning_core/cpu_smoke_result.json` | Stage AV-J CPU smoke；8 step 验证 dataset、四阶段 schedule、loss、eval variants、checkpoint 写入和结果 JSON，不提供能力结论 |
| `artifacts/omni_transformer_stage_avj_latent_reasoning_core/cpu_resume_smoke_result.json` | Stage AV-J CPU resume smoke；从 `cpu_smoke_checkpoints/latest.pt` 恢复后继续训练，验证 checkpoint/resume 链路 |
| `artifacts/omni_transformer_stage_avj_latent_reasoning_core/cuda_capacity_smoke_result.json` | Stage AV-J CUDA capacity smoke；d_model=192、layers=3、heads=4、batch=128、4,863,087 参数，验证 AMP、GPU 常驻数据、checkpoint 和 eval variants 可跑 |
| `artifacts/omni_transformer_stage_avj_latent_reasoning_core/capacity_70m_batch256_2step_result.json` | Stage AV-J 70M 档 CUDA capacity smoke；d_model=480、layers=8、heads=8、batch=256、71,694,351 参数，peak CUDA allocated 约 4,629.29 MB，验证本机可启动 70M 前后向 |
| `artifacts/omni_transformer_stage_avj_latent_reasoning_core/train_70m_result.json` | Stage AV-J 70M 长训结果；source record exact 约 99.8%，answer sequence exact 约 98.8%，但 target record exact 约 67.8%，判定不通过 |
| `artifacts/omni_transformer_stage_avj_latent_reasoning_core/train_70m_summary.json` | Stage AV-J 70M 长训指标汇总；记录 val/test/heldout 主指标、no-source/no-operation/no-process 消融、分任务 target/answer exact 与失败判断 |
| `artifacts/omni_transformer_stage_avj_latent_reasoning_core/train_70m_checkpoints/` | Stage AV-J 70M 长训 checkpoint 目录，包含本次训练的 latest/best 状态 |
| `artifacts/omni_transformer_stage_avj_latent_reasoning_core/*_checkpoints/` | Stage AV-J smoke/capacity checkpoint 目录；大型训练应另用 `train_70m_checkpoints` |
| `artifacts/omni_transformer_stage_avjb_trace_verifier/cpu_smoke_result.json` | Stage AV-J-B CPU smoke；验证 rich trace loss、copy gate、verifier、四阶段 schedule、checkpoint 和 eval variants 可跑，不提供能力结论 |
| `artifacts/omni_transformer_stage_avjb_trace_verifier/cpu_resume_smoke_result.json` | Stage AV-J-B CPU resume smoke；验证从 `cpu_smoke_checkpoints/latest.pt` 恢复训练 |
| `artifacts/omni_transformer_stage_avjb_trace_verifier/cuda_capacity_smoke_result.json` | Stage AV-J-B CUDA smoke；验证 AMP、GPU 常驻数据和 eval variants 可跑 |
| `artifacts/omni_transformer_stage_avjb_trace_verifier/capacity_70m_batch256_2step_result.json` | Stage AV-J-B 70M 档 CUDA capacity smoke；d_model=480、layers=8、heads=8、batch=256、72,161,403 参数，peak CUDA allocated 约 4,628.25 MB |
| `artifacts/omni_transformer_stage_avjb_trace_verifier/train_70m_result.json` | Stage AV-J-B 70M 长训结果；target record exact 约 69.5%，answer sequence exact 约 98.7%，candidate mask exact 约 41.8%，判定不通过 |
| `artifacts/omni_transformer_stage_avjb_trace_verifier/train_70m_summary.json` | Stage AV-J-B 70M 长训指标汇总；记录 val/test/heldout 主指标、candidate/count/copy/verifier 指标、消融、分任务指标、相对 AV-J 的小幅变化和失败判断 |
| `artifacts/omni_transformer_stage_avjb_trace_verifier/train_70m_checkpoints/` | Stage AV-J-B 70M 长训 checkpoint 目录，包含本次训练的 latest/best 状态 |
| `artifacts/omni_transformer_stage_avjb_trace_verifier/*_checkpoints/` | Stage AV-J-B smoke/capacity checkpoint 目录；正式长训结果在 `train_70m_checkpoints` |
| `artifacts/omni_transformer_stage_avjc_rl_verifier_reward/cpu_smoke_result.json` | Stage AV-J-C CPU smoke；验证 RL/verifier reward 阶段、policy-gradient stats、eval variants、JSON 输出和 checkpoint 写入 |
| `artifacts/omni_transformer_stage_avjc_rl_verifier_reward/cpu_resume_smoke_result.json` | Stage AV-J-C CPU resume smoke；验证从 `cpu_smoke_checkpoints/latest.pt` 继续训练 |
| `artifacts/omni_transformer_stage_avjc_rl_verifier_reward/capacity_70m_rl_2step_result.json` | Stage AV-J-C 70M RL capacity smoke；d_model=480、layers=8、heads=8、batch=256、72,161,403 参数，peak CUDA allocated 约 3,279.69 MB |
| `artifacts/omni_transformer_stage_avjc_rl_verifier_reward/rl_from_avjb_70m_result.json` | Stage AV-J-C 从 AV-J-B checkpoint 继续 RL 的 70M 长训结果；test target record exact 75.24%，candidate mask 43.44%，no-source target 31.47%，部分提升但不通过 |
| `artifacts/omni_transformer_stage_avjc_rl_verifier_reward/rl_from_avjb_70m_summary.json` | Stage AV-J-C checkpoint->RL 指标汇总；记录 reward history、val/test/heldout、相对 AV-J-B 的 target record 提升和 shortcut 风险 |
| `artifacts/omni_transformer_stage_avjc_rl_verifier_reward/train_70m_result.json` | Stage AV-J-C 从头 SFT+RL 的 70M 长训结果；test target record exact 65.87%，低于 AV-J-B，不通过 |
| `artifacts/omni_transformer_stage_avjc_rl_verifier_reward/train_70m_summary.json` | Stage AV-J-C 从头 SFT+RL 指标汇总；记录 candidate mask 未突破、no-source target shortcut 变强和失败判断 |
| `artifacts/omni_transformer_stage_avjc_rl_verifier_reward/*_checkpoints/` | Stage AV-J-C smoke/capacity/正式训练 checkpoint 目录，包括 `rl_from_avjb_70m_checkpoints` 与 `train_70m_checkpoints` |
...
| `docs/omni-transformer-stage-avj-latent-reasoning-core.md` | Stage AV-J 潜空间状态推理核心训练报告，记录 record 级状态变换任务族、teacher trace、四阶段 schedule、CPU/CUDA smoke、测试、70M capacity、70M 长训负结果、通过门槛与下一步 AV-J-B 修复方向 |
| `docs/omni-transformer-stage-avjb-trace-verifier.md` | Stage AV-J-B 长链 trace/verifier 训练报告，记录 rich trace、copy-vs-update gate、record verifier、answer loss 后移、CPU/CUDA smoke、70M capacity、70M 长训负结果、门槛和 AV-J-C 修复方向 |
| `docs/omni-transformer-stage-avjc-rl-verifier-reward.md` | Stage AV-J-C RL/verifier reward 训练报告，记录 oracle verifier reward、self-critical policy gradient、answer loss 关闭、CPU/resume/70M capacity smoke、两条 70M 长训负结果和 AV-J-D 方向 |
...
| Stage AV-J 潜空间状态推理核心训练入口、record 级状态变换任务、teacher trace、CPU/CUDA smoke、70M capacity 和 70M 长训负结果 | `docs/omni-transformer-stage-avj-latent-reasoning-core.md`、`experiments/omni_transformer_stage_avj_latent_reasoning_core.py`、`tests/test_stage_avj_latent_reasoning_core.py`、`artifacts/omni_transformer_stage_avj_latent_reasoning_core/cpu_smoke_result.json`、`artifacts/omni_transformer_stage_avj_latent_reasoning_core/cpu_resume_smoke_result.json`、`artifacts/omni_transformer_stage_avj_latent_reasoning_core/cuda_capacity_smoke_result.json`、`artifacts/omni_transformer_stage_avj_latent_reasoning_core/capacity_70m_batch256_2step_result.json`、`artifacts/omni_transformer_stage_avj_latent_reasoning_core/train_70m_result.json` 与 `artifacts/omni_transformer_stage_avj_latent_reasoning_core/train_70m_summary.json` |
| Stage AV-J-B 长链 trace/verifier、copy-vs-update gate、字段级 target 指标、answer loss 后移和 70M 长训负结果 | `docs/omni-transformer-stage-avjb-trace-verifier.md`、`experiments/omni_transformer_stage_avjb_trace_verifier.py`、`tests/test_stage_avjb_trace_verifier.py`、`artifacts/omni_transformer_stage_avjb_trace_verifier/cpu_smoke_result.json`、`artifacts/omni_transformer_stage_avjb_trace_verifier/cpu_resume_smoke_result.json`、`artifacts/omni_transformer_stage_avjb_trace_verifier/cuda_capacity_smoke_result.json`、`artifacts/omni_transformer_stage_avjb_trace_verifier/capacity_70m_batch256_2step_result.json`、`artifacts/omni_transformer_stage_avjb_trace_verifier/train_70m_result.json` 与 `artifacts/omni_transformer_stage_avjb_trace_verifier/train_70m_summary.json` |
| Stage AV-J-C RL/verifier reward、oracle record reward、self-critical policy gradient、answer loss 关闭和两条 70M 长训负结果 | `docs/omni-transformer-stage-avjc-rl-verifier-reward.md`、`experiments/omni_transformer_stage_avjc_rl_verifier_reward.py`、`tests/test_stage_avjc_rl_verifier_reward.py`、`artifacts/omni_transformer_stage_avjc_rl_verifier_reward/cpu_smoke_result.json`、`artifacts/omni_transformer_stage_avjc_rl_verifier_reward/cpu_resume_smoke_result.json`、`artifacts/omni_transformer_stage_avjc_rl_verifier_reward/capacity_70m_rl_2step_result.json`、`artifacts/omni_transformer_stage_avjc_rl_verifier_reward/rl_from_avjb_70m_result.json`、`artifacts/omni_transformer_stage_avjc_rl_verifier_reward/rl_from_avjb_70m_summary.json`、`artifacts/omni_transformer_stage_avjc_rl_verifier_reward/train_70m_result.json` 与 `artifacts/omni_transformer_stage_avjc_rl_verifier_reward/train_70m_summary.json` |
