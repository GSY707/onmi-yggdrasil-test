...
V2.1 阶段修订：V2-A 只验证推理介质，V2-B 只验证静态多模态 Boundary-MoE、Attention Pump 与 FFN-MoE；人类目标保持、自主 READ/EMIT、真实工具、工作树/记忆、多层多线程、树图通信、全双工调度和离线能力演化统一后移到 V2-C 整机实验，不再拆成彼此独立的 A1-A4 路线。
...
V2 的模型核心成立后，V2-C 再把 Boundary-MoE 扩展成系统级全双工拓扑：多个共享目标但状态隔离的节点按层级和线程运行，树形关系定义责任与授权，受控横向边传递必要事实，输入/输出队列允许环境事件、工具调用和子任务并发推进。该系统级扩展与局部模态 Boundary experts、核心内部 FFN experts 是三个不同层次，不能混写。
...
这里必须区分原则与实验：人类主权和授权边界从所有阶段开始生效，不等待模型学会；模型能否理解、保持、分解和响应人类目标则是 V2-C 的经验能力 Gate。V2-A/V2-B 的合成任务目标只用于验证模型机制，不能被写成人类目标对齐证据。
...
该图是单个推理节点的局部模型视图，不等于完整运行拓扑。V2-A/V2-B 只需关闭这一路径的推理介质和静态多模态边界；多节点、全双工消息、真实工具、人类目标连续性和工作树恢复由 [`v2-c-hierarchical-full-duplex-agent-experiment.md`](v2-c-hierarchical-full-duplex-agent-experiment.md) 统一验证。
...
本节定义目标调用协议。V2-B 只验证静态受控输入、固定输出 expert 和因果边界；教师调用、自主调用、工具等待、错误恢复和并发消息均在 V2-C 进入行为 Gate。
...
`READ / REASON / AUDIT / EMIT / STOP` 是每个节点内部的显式状态机，不要求整个系统串行停在同一阶段。V2-C 的全双工发生在系统层：不同节点可以分别读取、推理、等待工具和输出；同一节点的输入/输出队列彼此独立，并能在安全点接收更高权限的修正、取消、超时和新证据。

全双工不授权模型自己发明并发控制。队列、背压、租约、版本、时间、取消、幂等和不可逆动作审批由代码维护；模型学习目标语义、任务分解、读取意图、工具选择、冲突判断和重规划。树形父子关系维持责任链，受控横向边只传必要事实、资源和冲突信号。
...
主动调用属于 V2-C，按以下顺序训练；V2-B 只完成第 1 项静态融合：
...
V2-C 整机稳定后才允许离线固化 Boundary expert、FFN expert、adapter 或工作配置：
...
V2-C 将工作树的按需展开推广到模型运行层：节点可通过 READ 控制事件主动选择来源、区域、分辨率和专家。工作树负责目标、责任、分支、承诺、证据与恢复，记忆/RAG 负责跨任务来源和能力入口，provider KV 管理负责物理缓存；三者不得再被视为同一棵树。

任何 KV 释放都只表示取消激活。若后续状态依赖被删除 KV，必须重算或从工作树恢复，不能宣称指针 drop 天然无损。

## 14. V2-C 的渐进生成与资源自适应

渐进生成保留为 V2-C 整机能力，但推理深度、读取精度和渲染质量不再强制正比。
...
5. 自然语言 readout 通过因果忠实性门禁。

**V2-C 多层、多线程、树图混合全双工智能体闭环：**

1. 根节点能理解并跨线程、跨窗口保持人类目标、硬约束、修正、取消和停止条件。
2. 多层节点按不同抽象和时间尺度分工，多线程并发在相同总预算下形成质量、延迟或成本净收益。
3. 树形责任与授权链保持唯一，受控横向图连接具有独立边际价值且不扩大权限。
4. 节点输入/输出全双工运行，能处理异步事件、工具等待、超时、背压、陈旧消息和错误恢复。
5. READ/REASON/AUDIT/EMIT/STOP 在节点内显式，在整机中可并发协调；至少一种真实工具、一种生成/表达输出和一种动作路径通过正式验证。
6. 工作树、记忆/RAG、来源、工具结果和 provider KV 各守边界，并支持跨进程恢复。
7. 离线候选专家或工作配置通过独立评测、人类授权、版本化晋升和回滚；在线经验不能直接修改生产权重。

只有模型核心闭环与 V2-C 整机闭环都通过，才能称为“Project-Yggdrasil V2 架构完整版”。任何局部模型指标或单个 Agent 演示都不能跨过该边界。
...
| H6 人类目标能被跨线程保持和修正 | 单线程/层级、正常/中断/修正/取消/权限冲突 | 目标边界或恢复协议失败；停止真实外部动作 |
| H7 多层全双工运行降低关键路径成本 | flat、tree-sync、tree-duplex，同模型/工具/总预算 | 层级、并发或调度没有净收益，应删除相应复杂度 |
| H8 树图混合优于纯树 | tree-duplex vs graph-duplex，保留冲突和消息成本 | 横向边没有价值或增加竞态，应回退责任树 |
| H9 自主专家与工具调用降低总成本 | 静态全读 vs 主动 READ/工具，同质量下的调用与延迟 | query policy、索引、工具或成本设计失败 |
| H10 audit readout 忠实 | latent 干预、删除、打乱与行为一致性 | readout 只是事后合理化，不能作为安全证据 |
| H11 离线专家晋升提高长期表现 | retrieval/adapter/static/new expert 的独立任务、安全回归和真实指标 | 候选专家不应晋升，旧版继续服务 |
...
| integrated-system | V2-C 多层多线程树图全双工 Agent、人类目标、工作树/记忆、恢复、工具、主动调用和输出闭环 |
...
| 级联主动采样 | 保留为 V2-C 主动 READ 的组成部分，不是当前介质或静态多模态 Gate |
...
其整机形态由 V2-C 关闭：以人类目标为唯一责任根，多层节点按抽象和时间尺度分工，多线程通过全双工输入/输出并发推进，树形关系维持责任与授权，受控图边协调必要事实，局部 Boundary experts、系统级节点、FFN experts 与工具保持不同边界。
